...
export default function ModalArtistic() {
    const modalIsVisible = useSelector((state: { modal: modalState }) => state.modal.modalPregnancy);
    ...
    return (
        <section ref={modalRef} className={`${modalIsVisibleClass} w-full h-full relative flex flex-col gap-4 ${modalPhotosIsOpenClass}`}>
            <div className="flex relative w-full justify-between">
                <h3 className="text-4xl">Maman & bébé</h3>
            ...
            <div className="w-full grid grid-cols-4 gap-4 mx-auto md:grid-cols-2 sm:grid-cols-1 p-6 rounded-3xl">
                {data.imagePregnancy.map((imgs:{id:number;link:string;cameraLink:string;latitude:string;longitude:string;softwareLink:string}) => {
                    return (
                        <div key={imgs.id} className="relative">
                            ...
                            <Image alt={"Photo"} src={imgs.link} width={500} height={500} quality={60} className="h-full w-full rounded-3xl hover:scale-105 cursor-pointer transition ease-in-out duration-300" style={{ objectFit: "cover" }} onClick={() => openModalPhotos({ nameLinkPhotosUp: imgs.link, cameraLinkPhotosUp: imgs.cameraLink, longitudePhotosUp: imgs.longitude, softwareLinkPhotosUp: imgs.softwareLink, latitudePhotosUp: imgs.latitude, previousModal: "Pregnancy"})} onLoad={() => handleImageLoad()}/>
                        </div>
                    );
                })}
            </div>
