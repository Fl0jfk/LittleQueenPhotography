...
export default function ModalArtistic() {
    const modalIsVisible = useSelector((state: { modal: modalState }) => state.modal.modalBoudoirTexture);
    ...
    return (
        <section ref={modalRef} className={`${modalIsVisibleClass} w-full h-full relative flex flex-col gap-4 ${modalPhotosIsOpenClass}`}>
            <div className="flex relative w-full justify-between">
                <h3 className="text-4xl">Boudoir & Texture</h3>
                ...
                {data.imageBoudoirTexture.map((imgs:{id:number;link:string;cameraLink:string;latitude:string;longitude:string;softwareLink:string}) => {
                    return (
                        <div key={imgs.id} className="relative">
                            ...
                            <Image alt={"Photo"} src={imgs.link} width={500} height={500} quality={60} className="h-full w-full rounded-3xl hover:scale-105 cursor-pointer transition ease-in-out duration-300" style={{ objectFit: "cover" }} onClick={() => openModalPhotos({ nameLinkPhotosUp: imgs.link, cameraLinkPhotosUp: imgs.cameraLink, longitudePhotosUp: imgs.longitude, softwareLinkPhotosUp: imgs.softwareLink, latitudePhotosUp: imgs.latitude, previousModal: "BoudoirTexture"})} onLoad={() => handleImageLoad()}/>
                        </div>
                    );
                })}
            </div>
